...
# Sampling
d <- df[sample(1:nrow(df), size = 5000),] %>%
  as.data.frame()
...
plot_ly()  %>%
  ...
  add_annotations(text = "5",
                  ...
                  yref = "paper",
                  showarrow = FALSE) %>%
  add_annotations(text = paste("<sup>1</sup> NEO Basics - The Center for Near-Earth Object Studies (CNEOS)",
                               paste("#30DayChartChallenge Day 14",
                                     "Viz: Rami Krispin",
                                     "Data: The Center for Near-Earth Object Studies (CNEOS)",
                                     sep = " | "),
                               sep = "<br>"),
                  font = list(color = "white",
                              size = 13,
